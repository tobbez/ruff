...
impl<'ast, 'buf, 'fmt, 'src> DocstringLinePrinter<'ast, 'buf, 'fmt, 'src> {
            ...
            match action {
                ...
                CodeExampleAddAction::Format { mut kind } => {
                    let Some(formatted_lines) = self.format(kind.code())? else {
                        // Since we've failed to emit these lines, we need to put
                        // them back in the queue but have them jump to the front
                        // of the line to get processed before any other action.
    ...
    fn format(
        ...
        let (unformatted_first, unformatted_last) = match (code.first(), code.last()) {
...
impl<'src> CodeExampleRstLiteralBlock<'src> {
    ...
    fn new(original: InputDocstringLine<'src>) -> Option<CodeExampleRstLiteralBlock> {
        ...
        if rest.starts_with(".. ") {
            if let Some(litblock) = CodeExampleRstLiteralBlock::new_code_block(original) {
    ...
    fn new_code_block(original: InputDocstringLine<'src>) -> Option<CodeExampleRstLiteralBlock> {
    ...
    /// prefix. Based on the definition of minimum indentation, this implies
    /// there is at least one line in the slice returned with no whitespace
    /// prefix.
    fn code(&mut self) -> &[CodeExampleLine<'src>] {
    ...
    fn add_code_line(
        ...
        if rest.is_empty() {
            ...
            if let Some(next) = original.next {
                ...
                if indentation_length(next_indent) <= self.opening_indent {
                    queue.push_back(self.into_format_action());
                    return None;
                }
            } else {
        ...
        if indent_len <= self.opening_indent {
            ...
            // And indeed, reST is happy to let literal blocks have a first
            // line whose indentation is greater than a subsequent line in the
            // block. The only real restriction is that every line in the block
            // must be indented at least past the indentation of the `::` line.
...
fn indentation_trim(indent_len: TextSize, mut line: &str) -> &str {
